...
if (-not $SkipServer -and -not $TunnelOnly) {
    ...
    $VenvPython = "C:\Users\gene\AppData\Local\Python\pythoncore-3.14-64\python.exe"

    if (-not (Test-Path $VenvMarker)) {
        $VenvPython = "C:\Users\gene\AppData\Local\Python\pythoncore-3.14-64\python.exe"
        Write-Host ">> Phase 1a/5: installing Python dependencies (~1 min)... $VenvPython" -ForegroundColor Cyan
