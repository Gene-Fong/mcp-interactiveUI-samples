...
.DESCRIPTION
    Removes the HS Copilot's Azure footprint. Use this when:
    ...
    What this script DELETES (in dependency order):
      1. Container App           lob-mcp-apps-hs
    ...
    NOTE: the Container Apps environment, Log Analytics workspace, ACR, and
    managed identity are SHARED infra (same names as the sibling Salesforce
    app). If you also run the Salesforce app in this resource group, destroying
    them here tears down its infra too. This mirrors the Salesforce script's
    behavior exactly -- delete only the lob-mcp-apps-hs container app by hand
    if you want to keep the shared stack alive.
    ...
    What this script does NOT touch:
      ...
      - parameters.bicepparam         (local file with your HubSpot token -- untouched)
...
.NOTES
    ...
    the resources. Run from hubspot-crm/python/.
#>
...
$appRoot    = Split-Path -Parent $deployRoot
...
if (-not (Test-Path "$appRoot\pyproject.toml")) {
    Write-FailBlock -What "Run this from hubspot-crm/python/" `
