...
export function McpBridgeProvider({ appName, children }: { appName: string; children: React.ReactNode }) {
  ...
  // navigate — explicitly swap the rendered view from within a widget.
  // Widget-initiated tool calls are suppressed by widgetCallDepth (so drill-down
  // fetches don't clobber the current view), so a redirect after save must set
  // toolData directly rather than relying on ontoolresult firing.
  const navigate = useCallback((d: any) => { if (d) setToolData(d); }, []);
  ...
  return (
    <McpBridgeContext.Provider value={{
      toolData, theme, callTool, navigate, notifyHeight, openExternal,
