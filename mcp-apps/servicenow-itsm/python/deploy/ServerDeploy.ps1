<#
.SYNOPSIS
    ServerDeploy -- end-to-end deploy of the SN MCP server to Azure
    Container Apps, plus agent upload to MOS3.

.DESCRIPTION
    Use this script when you want the SN MCP server hosted in Azure
...
.NOTES
    ...
    rights on the resource group. Run from servicenow-itsm/python/.
#>
...
Write-Host "   SERVER DEPLOY -- SN Copilot to Azure" -ForegroundColor Cyan
Write-Host "  =====================================" -ForegroundColor DarkCyan
Write-Host ""
...
Assert-File -Path $paramsFile `
    ...
    -Hint "Setup step: edit $paramsFile and fill in your ServiceNow credentials, then re-run."
...
az containerapp update `
    ...
    --image "$acrLoginServer/sn-mcp-copilot:latest" | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-FailBlock -What "az containerapp update failed (exit $LASTEXITCODE)" `
        ...
        -Hint "Check that the image '$acrLoginServer/sn-mcp-copilot:latest' actually exists: az acr repository list -n $AcrName"
    exit 1
}

# 1e. Enable sticky sessions on ingress.
# SN's main.bicep does NOT declare stickySessions, so this CLI call is the
# primary way to enable affinity (not a safety net). Without sticky, MCP
# streamable-HTTP session state (in-memory per replica) is lost on
# round-robin requests and Teams sees Method Not Found / multi-accept /
# cyclic loops. Idempotent; safe to re-run.
Write-Host "   >> Enabling sticky sessions on ingress..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "  Next: open Microsoft Teams, pick Ask - ServiceNow, test." -ForegroundColor Gray
Write-Host ""
